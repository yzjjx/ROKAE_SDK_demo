...
// // 定义减速比
// const std::array<double, 6> reduction_ratio = {
//     120.0, -120.0, -120.0,
//     100.0, -100.0, 100.0
// };
// 定义减速比
...
void writeLogTxt(const std::string& filename, const std::vector<LogRow>& logs)
{
    ...
    for (const auto& row : logs)
    {
        ...
        // 速度
        ofs << "[";
        for (std::size_t i = 0; i < row.q_dot_m.size(); ++i)
        {
            ofs << row.q_dot_m[i];

            if (i + 1 < row.q_dot_m.size())
            {
                ofs << ",";
            }
        }
        ...
        // 位置
        ofs << "[";
        for (std::size_t i = 0; i < row.q_m.size(); ++i)
        {
            ofs << row.q_m[i];
        ...
        // 力矩乘以减速比
        ofs << "[";
        for (std::size_t i = 0; i < row.tau_m.size(); ++i)
        {
            double tau_real = row.tau_m[i] * reduction_ratio[i];

            ofs << tau_real;

            if (i + 1 < row.tau_m.size())
            {
                ofs << ",";
            }
        }
...
int main()
{
    ...
    try
    {
        ...
        SDU_SR4.calibrateForceSensor(true, 6, ec);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
        std::cout << "完成力传感器标定" << std::endl;
        ...
        std::function<rokae::JointPosition()> callback = [&]() -> rokae::JointPosition
        {
            ...
            (void)SDU_SR4.getStateData(rokae::RtSupportedFields::motorTau, row.tau_m);
